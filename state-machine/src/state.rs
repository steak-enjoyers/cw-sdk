...
/// The identifier of the chain. Used to prevent tx replay attacks.
...
///
/// NOTE: It makes more sense to use u64 rather than i64, as block heights
/// shouldn't be negative. However Tendermint ABCI uses i64 for some reason,
/// so we just follow their standard.
...
pub fn load_code_by_address(store: &dyn Storage, contract_addr: &Addr) -> Result<Binary> {
